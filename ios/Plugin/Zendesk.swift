...
@objc public class ZendeskImpl: NSObject {
    ...
    func parsePreChatFormConfig(_ key: String) -> FormFieldStatus {
        if (key == "required") {
    ...
    func preChatFormConfigurationFromConfig(options: [String: Any]) -> ChatFormConfiguration? {
        let nameStatus = parsePreChatFormConfig(options["name"] as! String)
        let emailStatus = parsePreChatFormConfig(options["email"] as! String)
        let phoneStatus = parsePreChatFormConfig(options["phone"] as! String)
        let departmentStatus = parsePreChatFormConfig(options["department"] as! String)
    ...
    @objc public func startChat(parentVC: UIViewController, options: [String: Any]) -> Void {
        ...
        let engines = try! [ChatEngine.engine()]
        ...
        presentViewController(parentVC: parentVC, vc: viewController)
    }
    ...
    @objc public func showHelpCenterFunction(parentVC: UIViewController, options: [String: Any]) {
        ...
        presentViewController(parentVC: parentVC, vc: helpCenter)
    }
    ...
    @objc public func presentViewController(parentVC: UIViewController, vc: UIViewController) {
        ...
        DispatchQueue.main.async {
            parentVC.present(self.presentedViewController!, animated: true, completion: nil)
        }
    }
}
